...
var bindOptions = function (stringToBoolean) {
    ...
    var optionItems = document.querySelectorAll('[bindOption]');
    var _loop_1 = function (i) {
        var optionItem = optionItems[i];
        var isCheckbox = optionItem.getAttribute('type') === 'checkbox';
        var isSelect = optionItem.tagName.toLowerCase() === 'select';
        var isInput = optionItem.tagName.toLowerCase() === 'input';
        var isRadio = optionItem.tagName.toLowerCase() === 'radio';
        var boundOption = optionItem.getAttribute('bindOption') || '';
        optionItem.addEventListener('change', function (e) {
            if (e.target) {
                if (isCheckbox) {
                    stardust.options[boundOption] = e.target.checked ? true : false;
                }
                else if (isSelect) {
                    var target = e.target;
...
var bindActions = function () {
    var itemsWithActions = document.querySelectorAll('[bindAction]');
    for (var i = 0; i < itemsWithActions.length; i++) {
        var boundElement = itemsWithActions[i];
        var actionItems = boundElement.getAttribute('bindAction');
        if (actionItems) {
